...
public class Live2DRenderer implements GLWallpaperService.Renderer
{
	...
	final String MOTION_PATH= LAppDefine.charaName + "/mtn/IDLING.mtn";
	final String PHYSICS_PATH= LAppDefine.charaName + "/" + LAppDefine.charaName + ".physics.json";
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config)
    {
		...
		try
		{
			InputStream in = mngr.open( LAppDefine.MODEL_MOC) ;
    	...
    	try
		{
			InputStream in = mngr.open( LAppDefine.MODEL_JSON) ;
		...
		try
		{
			...
			for (int i = 0 ; i < modelSetting.getTextureFiles().length; i++){
				InputStream in = mngr.open( LAppDefine.charaName + "/" + modelSetting.getTextureFile(i) ) ;
    ...
    public boolean hitTest(String id,float testX,float testY)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			{
				System.out.println("test " + id + "=" + hitTestSimple(modelSetting.getHitAreaID(i),testX,testY));
    ...
    public boolean hitTestSimple(String drawID,float testX,float testY)
	{
        ...
        System.out.println(" area pos:  top="+top+" bottom="+bottom + "; test pos: Y="+ty);
